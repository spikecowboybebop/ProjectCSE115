void patient_delete_records() {
	...
	char count[10];
	FILE *rdptr;
	rdptr = fopen("patient_count.txt", "r");
	...
	char userdata[c][70];
	FILE *getdata;
	getdata = fopen("patient_data.txt", "r");
	...
	FILE *writedata;
	writedata = fopen("patient_data.txt", "w");
	for(int i = 0; i < c; i++) {
		if(cmpr[i] == 0) {
			continue;
		}
	...
	c--;
	FILE *changecounter;
	changecounter = fopen("patient_count.txt", "w");
